...
.form-row select {
    ...
    height: auto;
    border-color: 2px solid; /* Resets the height to auto */
}
...
.form-row input[type="text"]:focus {
  border-color: #2196F3; /* Change the border color when focused */
  box-shadow: 0 0 5px rgba(33, 150, 243, 0.5); /* Add a box shadow when focused */
...
<table id="yourTableID" class="display">
    ...
    <tbody>
        <?php include 'findprocess2.php';  ?>

       <?php while ($row = $ekonsulta_record->fetch_assoc()): ?>
    <tr>
        <td style="font-weight: bold;"><?= strtoupper($row['ENLIST_DATE']) ?></td>
        <td style="font-weight: bold;"><?= strtoupper($row['PX_LNAME']) ?></td>
        <td style="font-weight: bold;"><?= strtoupper($row['PX_FNAME']) ?></td>
        <td style="font-weight: bold;"><?= strtoupper($row['PX_MNAME']) ?></td>
        <td style="font-weight: bold;"><?= strtoupper($row['PX_PIN']) ?></td>
...
<div class="form-row">
 ...
 <div class="form-row">
   ...
   <input  type="Date"  id="dateekon"   name="ekondate" value="<?php echo date("m-d-Y"); ?>">
            ...
            <button type="submit" name="updatekonsulta" class="submit-button">
