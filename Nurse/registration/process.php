...
if (isset($_POST['submit'])) {
  ...
  // Display a success message and redirect
       ...
       <body>
        ...
        $phic = $_POST['phic'];
        $ekonsulta = $_POST['ekonsulta'];
        $ekondate = $_POST['ekondate'];

        $date = date("m-d-Y");

        $sql = "INSERT INTO nurse_medical_record (record_no, name, age, address, sex, status, birthplace, occupation, birthday, religion, contact_no, date, phic, ekonsulta, ekondate)
                VALUES ('$record_no', '$name', '$age', '$address', '$sex', '$status', '$birthplace', '$occupation', '$birthday', '$religion', '$contact_no', '$date', '$phic', '$ekonsulta','$ekondate')";
...
$sql = "SELECT * FROM nurse_medical_record ORDER BY id DESC LIMIT 10";
    ...
    if ($db->query($sql) === TRUE) {
        ...
        <body>
          <div class="overlay">
            ...
            echo '<script>
                setTimeout(function() {
                    window.location.href = "ekonsulta.php";
                }, 3000); // Redirect to registration.php after 3 seconds
            </script>';
    } else {
        echo "Error updating record: " . $db->error;
